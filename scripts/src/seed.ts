...
async function seed() {
  ...
  const [student1] = await db.insert(usersTable).values({
    ...
    totalPoints: 450,
    streak: 7,
    badges: ["first_lesson", "streak_7", "points_100"],
  }).returning();
  ...
  const [student3] = await db.insert(usersTable).values({
    ...
    totalPoints: 820,
    streak: 15,
    badges: ["first_lesson", "streak_7", "perfect_quiz", "points_100", "points_500"],
  }).returning();
  ...
  const [course1] = await db.insert(coursesTable).values({
    ...
    description: "Start your English learning journey with fundamental vocabulary, basic grammar, and everyday conversations.",
    level: "A1",
  ...
  const [course2] = await db.insert(coursesTable).values({
    ...
    description: "Build on your basic English skills with expanded vocabulary and more complex sentence structures.",
    level: "A2",
    teacherId: teacher1.id,
    price: "349",
    isActive: true,
  ...
  const [course3] = await db.insert(coursesTable).values({
    ...
    description: "Develop your ability to handle most situations likely to arise while travelling and communicate more fluently.",
    level: "B1",
  ...
  const [course4] = await db.insert(coursesTable).values({
    title: "Upper-Intermediate English (B2)",
    description: "Express yourself with confidence on a wide range of topics and engage with native speakers.",
    level: "B2",
    teacherId: teacher2.id,
    price: "449",
    isActive: true,
  ...
  await db.insert(lessonsTable).values([
    ...
    { moduleId: mod1.id, title: "Vocabulary: Greetings and Farewells", type: "document", content: "# Greetings\n- Hello / Hi\n- Good morning\n- Good afternoon\n- Good evening\n- Goodbye / Bye\n- See you later!", duration: 5, order: 2 },
    { moduleId: mod1.id, title: "Introduction Quiz", type: "quiz", content: null, duration: 15, order: 3 },
    { moduleId: mod2.id, title: "Numbers 1-100", type: "video", content: "https://www.youtube.com/embed/dQw4w9WgXcQ", duration: 12, order: 1 },
    { moduleId: mod2.id, title: "Colors Vocabulary", type: "text", content: "Learn the 12 basic colors in English: red, blue, green, yellow, orange, purple, pink, white, black, brown, grey, gold.", duration: 8, order: 2 },
    ...
    { moduleId: mod3.id, title: "Daily Activities Reading", type: "document", content: "# Daily Activities\nI wake up at 7 AM. I brush my teeth and take a shower. I have breakfast at 8 AM.", duration: 10, order: 2 },
  ]);

  // Modules for B1 course
  ...
  await db.insert(lessonsTable).values([
    ...
    { moduleId: bmod1.id, title: "Present Perfect vs Past Simple", type: "document", content: "# Present Perfect vs Past Simple\n\nUse **present perfect** for experiences: I have visited London.\nUse **past simple** for completed actions: I visited London last year.", duration: 15, order: 2 },
    { moduleId: bmod2.id, title: "At the Airport", type: "video", content: "https://www.youtube.com/embed/dQw4w9WgXcQ", duration: 18, order: 1 },
    { moduleId: bmod2.id, title: "Hotel Check-in", type: "audio", content: "https://example.com/audio.mp3", duration: 10, order: 2 },
  ]);
  ...
  const futureDate2 = new Date(Date.now() + 5 * 24 * 60 * 60 * 1000); // 5 days from now
  const futureDate3 = new Date(Date.now() + 1 * 24 * 60 * 60 * 1000); // Tomorrow

  await db.insert(liveClassesTable).values([
    {
      title: "A1 Grammar Workshop - Articles",
    ...
    {
      title: "B1 Conversation Practice",
      description: "Practice your spoken English in a supportive group environment",
      teacherId: teacher2.id,
      courseId: course3.id,
    ...
    },
    {
      ...
      description: "One-on-one session tailored to your specific needs",
      teacherId: teacher1.id,
      courseId: null,
  ...
  await db.insert(questionsTable).values([
    ...
    { quizId: quiz1.id, type: "multiple_choice", question: "What is the correct response to 'How are you?'", options: ["I am fine, thank you.", "My name is John.", "I am 25 years old.", "I live in London."], correctAnswer: "I am fine, thank you.", points: 10, order: 4 },
    { quizId: quiz1.id, type: "true_false", question: "'Hi' is more formal than 'Hello'.", options: ["True", "False"], correctAnswer: "False", points: 10, order: 5 },
  ]);

  console.log("Quizzes created");
  console.log("\n✅ Database seeded successfully!");
  console.log("\nDemo accounts:");
  console.log("  Admin:   admin@sphereenglish.com / admin123");
  console.log("  Teacher: sarah.johnson@sphereenglish.com / teacher123");
  console.log("  Teacher: michael.brown@sphereenglish.com / teacher123");
  console.log("  Student: alice@example.com / student123");
  console.log("  Student: bob@example.com / student123");
  console.log("  Student: ceren@example.com / student123");
